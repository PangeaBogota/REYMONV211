...
app_angular.controller("pedidoController",['Conexion','$scope','$location','$http','$routeParams','$timeout',function (Conexion,$scope,$location,$http,$routeParams,$timeout) {
	...
	$scope.onChangeTercero=function(){
		...
		$scope.list_precios=[];
		debugger
		CRUD.select("select  codigo_sucursal||'-'||nombre_sucursal as sucursal,*from erp_terceros_sucursales where rowid_tercero = '"+$scope.terceroSelected.rowid+"'   order by codigo_sucursal",function(elem){ debugger;$scope.list_Sucursales.push(elem)})
